...
REQUIRED_VIDEO_PLUGIN_FILES = $(PLUGINS_DIR)/$(ANGRYLION_RDP_LIB)
REQUIRED_RSP_PLUGIN_FILES = $(PLUGINS_DIR)/$(RSP_CXD4_LIB)
endif
...
STATIC_LIBRARIES += ../../../$(RSP_LIB_STATIC)
...
STATIC_LIBRARIES += ../../../$(ANGRYLION_RDP_LIB_STATIC)
REQUIRED_VIDEO_PLUGIN_FILES = $(ANGRYLION_RDP_LIB_STATIC)
STATIC_LIBRARIES += ../../../$(RSP_CXD4_LIB_STATIC)
REQUIRED_RSP_PLUGIN_FILES = $(RSP_CXD4_LIB_STATIC)
endif
...
$(PLUGINS_DIR)/$(GLIDEN_VIDEO_LIB) : $(GLIDEN_VIDEO_DIR)/$(GLIDEN_VIDEO_LIB_JS)
	mkdir -p $(PLUGINS_DIR)
	cp "$<" "$@"
...
$(GLIDEN_VIDEO_LIB_STATIC): .FORCE
	cd $(GLIDEN_VIDEO_DIR) && \
			emcmake cmake -DMUPENPLUSAPI=ON -DMUPENPLUSAPI_GLIDENUI=OFF -DEMSCRIPTEN=ON -DM64P_STATIC_PLUGINS=ON -DCMAKE_CXX_FLAGS="$(OPT_FLAGS) -DEMSCRIPTEN=1 -DUSE_GLES=1 -DUSE_SDL=2" -DCMAKE_C_FLAGS="$(OPT_FLAGS) -DEMSCRIPTEN=1 -sUSE_SDL=2" ../../src && \
			emmake $(MAKE)

$(RICE_VIDEO_LIB_STATIC): .FORCE
	cd $(RICE_VIDEO_DIR) && \
			emmake $(MAKE) \
			CROSS_COMPILE="" \
			POSTFIX=-web\
...
$(ANGRYLION_RDP_LIB_STATIC): .FORCE
	cd $(ANGRYLION_RDP_DIR) && \
	mkdir -p build && \
	cd build && \
	emcmake cmake -DCMAKE_CXX_FLAGS="$(OPT_FLAGS)" -DCMAKE_C_FLAGS="$(OPT_FLAGS)" .. -DM64P_STATIC_PLUGINS=ON -DGLES=ON && \
	$(MAKE)
	  VERBOSE=1 \
	  M64P_STATIC_PLUGINS=$(USE_STATIC_PLUGINS) \
...
clean-web:
	...
	rm -f $(GLIDEN_VIDEO_DIR)/$(GLIDEN_VIDEO_LIB)
	rm -f $(GLIDEN_VIDEO_DIR)/$(GLIDEN_VIDEO_LIB_JS)
	rm -f $(GLIDEN_VIDEO_DIR)/libmupen64plus-video-GLideN64-web.a
