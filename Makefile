...
OPT_LEVEL = -O3 -g3 -s ASSERTIONS=1 -s AGGRESSIVE_VARIABLE_ELIMINATION=1 -s NO_EXIT_RUNTIME -s ALLOW_MEMORY_GROWTH=1 -pthread -sPTHREAD_POOL_SIZE=8
...
REQUIRED_VIDEO_PLUGIN_FILES = $(ANGRYLION_RDP_LIB_STATIC) #$(RICE_VIDEO_LIB_STATIC)
REQUIRED_RSP_PLUGIN_FILES = $(RSP_CXD4_LIB_STATIC) # $(RSP_LIB_STATIC)
...
$(BIN_DIR)/$(TARGET_JS): $(INDEX_TEMPLATE) $(REQUIRED_PLUGINS) $(INPUT_FILES)
	...
	cd $(UI_DIR) && \
	 	emmake make \
			...
			-s USE_PTHREADS=1 \
			-s DEMANGLE_SUPPORT=1 -s MODULARIZE=1 \
...
clean-video:
	rm -f $(RICE_VIDEO_LIB_STATIC)
	rm -f $(RICE_VIDEO_DIR)/$(RICE_VIDEO_LIB_JS)
	rm -f $(ANGRYLION_RDP_DIR)/bin/$(ANGRYLION_RDP_LIB_STATIC)
